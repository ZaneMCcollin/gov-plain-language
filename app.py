...
with right:
    if convert_btn:
        ...
        # Hard-enforce Grade ≤ 8: do not save if any section is above target
        over = [(s.get("title","Section"), float(s.get("grade_en", 99.0) or 99.0)) for s in out_sections if float(s.get("grade_en", 99.0) or 99.0) > 8.0]
        if over:
            ...
            st.session_state.snapshot = snap
            log_audit(event="convert_failed_grade", user_email=AUTH_EMAIL, workspace=st.session_state.workspace, doc_id=st.session_state.doc_id, meta={"over": over_sorted})
            st.stop()
