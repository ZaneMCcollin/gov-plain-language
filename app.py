...
def _is_streamlit_cloud() -> bool:
    """
    Detect Streamlit Community Cloud runtime.

    Streamlit Cloud sets STREAMLIT_SERVER_HEADLESS=1.
    Local dev typically won't have it.
    """
    return os.environ.get("STREAMLIT_SERVER_HEADLESS", "").strip().lower() in ("1", "true", "yes")
...
# ============================================================
# ✅ Authentication (Email / Google Workspace) using Streamlit built-in auth (st.login, st.user)
# Supports named provider config like [auth.google]
# ============================================================
...
def require_login() -> str:
    """
    Enforces login if Streamlit auth is available.
    Uses a NAMED provider by default (google), matching secrets like:
      [auth]
      redirect_uri = "..."
      cookie_secret = "..."
      [auth.google]
    ...
    # If Streamlit auth isn't available (older Streamlit), don't block.
    if not hasattr(st, "login") or not hasattr(st, "user"):
        return ""

    try:
        # If already logged in, st.user.is_logged_in will be True
        ...
        # IMPORTANT: use provider name to match [auth.google]
