...
out_sections, _fix_report = _auto_resimplify_sections_to_grade8(
    out_sections,
    doc_id_scoped=scoped_doc_id(st.session_state.doc_id, st.session_state.workspace),
    user_email=AUTH_EMAIL,
...
if _fix_report:
    ...
    for r in _fix_report[:10]:
        st.write(f"- {r['title']}: {r['old_grade']} → {r['new_grade']} (rounds: {r['rounds']})")
...
over2 = [(s.get("title","Section"), float(s.get("grade_en", 99.0) or 99.0)) for s in snap["sections"] if float(s.get("grade_en", 99.0) or 99.0) > 8.0]
if over2:
    ...
    for t, g in over_sorted2:
        st.write(f"- {t}: {g}")
    log_audit(event="save_failed_grade", user_email=AUTH_EMAIL, workspace=st.session_state.workspace, doc_id=st.session_state.doc_id, meta={"over": over_sorted2})
    st.stop()
